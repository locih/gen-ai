...
- compare_periods: сравнить метрику между двумя периодами (delta, ratio)
...
7. Для сравнения метрики между двумя датами предпочитай compare_periods
   (во сколько раз вырос / на сколько п.п. изменился).
"""
...
def _append_trace(trace_path: Path, run_id: str, payload: dict) -> None:
    row = {
        "run_id": run_id,
    ...
    *,
    ...
    for step in range(1, max_iter + 1):
        ...
        if not msg.tool_calls:
            ...
            trace.append(entry)
            if trace_file:
                _append_trace(trace_file, run_id, entry)
            return _finish(
                {
                    "answer": msg.content,
                    "structured": None,
