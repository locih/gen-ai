...
# ===========================================================================
# 5. Сравнение двух периодов (ДЗ)
# ===========================================================================

_PERIOD_MONTH = re.compile(r"^\d{4}-\d{2}$")
_PERIOD_DAY = re.compile(r"^\d{4}-\d{2}-\d{2}$")
_FX_METRICS = {"fx_USD": "USD", "fx_EUR": "EUR", "fx_CNY": "CNY"}
_ALLOWED_METRICS = {"key_rate", "cpi", "unemployment"} | set(_FX_METRICS)
...
def _resolve_period(period: str) -> dict:
    ...
    if _PERIOD_MONTH.fullmatch(period):
        ...
        return {
            "label": period,
            "on_date": f"{y}-{m:02d}-15",
            "year": y,
            "month": m,
        }
    if _PERIOD_DAY.fullmatch(period):
    ...
    return {"error": f"период должен быть YYYY-MM или YYYY-MM-DD, получено: {period!r}"}
...
def _metric_point(metric: str, period: str) -> dict:
    ...
    if metric == "key_rate":
        obs = get_key_rate(p["on_date"])
        if "error" in obs:
            return obs
        return {
            "date": obs.get("valid_from") or obs["date"],
            "value": obs["rate"],
            "source": obs["source"],
        }

    if metric in _FX_METRICS:
        obs = get_fx_rate(_FX_METRICS[metric], p["on_date"])
        if "error" in obs:
            return obs
        return {"date": obs["date"], "value": obs["rate"], "source": obs["source"]}

    if metric == "cpi":
        obs = get_inflation(p["year"], p["month"])
        if "error" in obs:
            return obs
        return {
    ...
    if metric == "unemployment":
        obs = get_unemployment(p["year"], p["month"])
        if "error" in obs:
            return obs
        return {
            "date": f"{p['year']}-{p['month']:02d}",
            "value": obs["unemployment"],
            "source": obs["source"],
        }

    return {"error": f"метрика {metric} не поддерживается"}
...
def compare_periods(metric: str, period_a: str, period_b: str) -> dict:
    ...
    if "error" in a:
        return {"error": a["error"], "metric": metric, "period": period_a}
    b = _metric_point(metric, period_b)
    if "error" in b:
        return {"error": b["error"], "metric": metric, "period": period_b}
